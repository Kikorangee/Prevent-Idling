...
export default function App({ api: initialApi, registerFocus }) {
  ...
  const speedCfgRef = useRef({ limit: 100, durMs: 30000 });
  ...
  const applyStatusInfos = useCallback(statusInfos => {
    ...
    const { limit, durMs } = speedCfgRef.current;
    const spells = spellsRef.current;
    const nowMs = Date.now();
    const flagged = {};
    Object.entries(pos).forEach(([id, p]) => {
      if (limit > 0 && p.speed >= limit) {
        if (!spells[id]) spells[id] = nowMs;
        if (nowMs - spells[id] >= durMs) flagged[id] = { speed: p.speed, sinceMs: spells[id] };
      } else {
        delete spells[id];
      }
    });
  ...
  useEffect(() => {
    speedCfgRef.current = {
      limit: parseFloat(speedLimit) || 0,
      durMs: (parseInt(speedDuration, 10) || 0) * 1000
    };
    spellsRef.current = {};
    setSpeeding({});
  }, [speedLimit, speedDuration]);
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      const [chunks, statusInfos] = await Promise.all([
        apiMultiCall(api, calls),
        apiCall(api, "Get", { typeName: "DeviceStatusInfo" })
      ]);
      setEvents(chunks.flat());
      applyStatusInfos(statusInfos);
  ...
  const vehicleRows = useMemo(() => {
    ...
    for (const e of events) {
      ...
      const cur = byDev.get(id) || { id, name: names.device[id] || id, events: 0, hours: 0 };
      cur.events += 1;
      cur.hours += h;
      byDev.set(id, cur);
    }
  ...
  const mapEvents = useMemo(() => events.map(e => ({
    deviceId: e.device ? e.device.id : null,
    activeFrom: e.activeFrom,
    minutes: durationToHours(e.duration) * 60
  })), [events]);
