...
export default function MapPanel({
  ...
  speedLimit, setSpeedLimit, speedDuration, setSpeedDuration
}) {
    ...
    allIds.forEach(id => {
      ...
      if (isSpeeding) {
        html += `<span style="color:#d32f2f;font-weight:600">SPEEDING \u2014 ${Math.round(spd.speed)} km/h</span>` +
                ` (limit ${speedLimit} km/h, over for ${fmtSince(spd.sinceMs)})<br>`;
      }
      if (idle) {
  ...
  return (
    <div style={{ margin: "20px 0" }}>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 12, marginBottom: 8, flexWrap: "wrap" }}>
        ...
        <TextInput label="Speed limit (km/h)" value={speedLimit} onChange={e => setSpeedLimit(e.target.value)} />
      ...
      <div style={{ display: "flex", gap: 18, fontSize: 12, color: "#5b6b7b", marginBottom: 6, flexWrap: "wrap" }}>
        <span><i style={dotStyle(SEVERITY.high)} /> Flashing = speeding now</span>
